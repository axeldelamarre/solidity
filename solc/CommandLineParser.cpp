...
General Information)").c_str(),
	...
	m_options.coloredOutput = !m_args.count(g_argNoColor) && (isatty(STDERR_FILENO) || m_args.count(g_argColor));
	...
	solAssert(m_options.inputMode == InputMode::CLI, "");
	...
	if (m_args.count(g_argImportAst) > 0)
		m_options.inputMode = InputMode::ASTImport;
	else
